...
func (db *DB) IterateLoopSegments(ctx context.Context, opts IterateLoopSegments, fn func(context.Context, LoopSegmentsIterator) error) (err error) {
	...
	return db.ChooseAdapter(uuid.UUID{}).LoopSegments(ctx, db.aliasCache, opts, fn)
}

// LoopSegments postgres implementation.
func (p *PostgresAdapter) LoopSegments(ctx context.Context, aliasCache *NodeAliasCache, opts IterateLoopSegments, fn func(context.Context, LoopSegmentsIterator) error) (err error) {
...
type spannerLoopSegmentIterator struct {
	...
	batchSize int
	// TODO would be nice to have it at some point
	// batchPieces are reused between result pages to reduce memory consumption
	// batchPieces []Pieces
...
// LoopSegments spanner implementation.
func (s *SpannerAdapter) LoopSegments(ctx context.Context, aliasCache *NodeAliasCache, opts IterateLoopSegments, fn func(context.Context, LoopSegmentsIterator) error) (err error) {
...
func (it *spannerLoopSegmentIterator) scanItem(ctx context.Context, item *LoopSegmentEntry) (err error) {
	...
	item.StreamID, err = uuid.FromBytes([]byte(streamID))
	if err != nil {
		return err
	}
