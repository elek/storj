...
func (service *Service) Claim(ctx context.Context, userID uuid.UUID) (address blockchain.Address, err error) {
	...
	if err = service.walletsDB.Add(ctx, userID, address); err != nil {
		return
	}
	return
}
...
func (service *Service) Get(ctx context.Context, userID uuid.UUID) (address blockchain.Address, err error) {
	defer mon.Task()(&ctx)(&err)
