...
func TestStore_MergeRecordsWhenCompactingWithLostPage(t *testing.T) {
	...
	checkIndex := func(k Key, epi, eri uint64) {
		t.Helper()
		gpi, gri := s.tbl.index(keyIndex(&k) & s.tbl.mask)
		assert.That(t, gpi == epi && gri == eri)
	}
	...
	// ensure they are going to be placed where they need to be for the test to make sense.
	checkIndex(k0, 0, rPerP-1)
	checkIndex(k1, 0, rPerP-1)
