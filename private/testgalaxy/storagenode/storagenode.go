...
import (
	...
	"path/filepath"
	"runtime/pprof"
	"storj.io/storj/private/testgalaxy/common"
	"strconv"
	"strings"
	"time"
...
func newStorageNodes(ctx context.Context, log *zap.Logger, count int, whitelistedSatellites storj.NodeURLs) (_ []common.ClosablePeer, err error) {
	...
	for i := 0; i < count; i++ {
		...
		peers = append(peers, common.NewClosablePeer(system))
	}
	return peers, nil
}

func newStorageNode(ctx context.Context, prefix string, index, count int, log *zap.Logger, sources []trust.Source) (_ *StorageNode, err error) {
	defer mon.Task()(&ctx)(&err)
...
//func (planet *Planet) FindNode(nodeID storj.NodeID) *storagenode.StorageNode {
...
//func StopNodeAndUpdate(ctx context.Context, node *storagenode.StorageNode) (err error) {
//	defer mon.Task()(&ctx)(&err)
//
