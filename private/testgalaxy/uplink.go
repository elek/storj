...
func (client *Uplink) DialMetainfo(ctx context.Context, destination common.Peer, apikey *macaroon.APIKey) (_ *metaclient.Client, err error) {
	defer mon.Task()(&ctx)(&err)
	return metaclient.DialNodeURL(ctx, client.Dialer, destination.NodeURL().String(), apikey, "Test/1.0")
}

// DialPiecestore dials destination storagenode and returns a piecestore client.
func (client *Uplink) DialPiecestore(ctx context.Context, destination common.Peer) (_ *piecestore.Client, err error) {
	defer mon.Task()(&ctx)(&err)
